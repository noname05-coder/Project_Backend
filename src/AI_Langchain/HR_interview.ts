...
async function generate_summery(chat_history: ChatHistoryEntry[], interviewee_data: IntervieweeData) {
  try {
...
async function getData(){
  const response = await axios.post("http://localhost:3000/api/v1/hr/hr_data");  
  return response.data as IntervieweeData;
}
...
async function interview() {
  ...
  let role_data : IntervieweeData= {
    ...
    experience: "2.5 years at a product-based startup working on image classification and NLP projects",
    company_applying: "Meta",
    job_description: "Meta is looking for a Machine Learning Engineer with experience in deep learning, computer vision, and NLP. The candidate should have a strong understanding of algorithms and data structures, as well as experience with TensorFlow or PyTorch.",
...
const chain2 = new ConversationChain({
  ...
  const displayChatHistory = async() => {
    ...
    try {
      const data = await generate_summery(chatHistory, role_data);
  ...
  try {
    ...
    say.speak(response.output, "Alex", 0.5, (err) => {
      if (err) {
    ...
    say.speak(followUpResponse.output, "Alex", 0.5, (err) => {
      if (err) {
        console.error("Error speaking:", err);
      }
    });
  ...
  while (continueInterview) {
    ...
    try {
      ...
      const response = await chain2.invoke({
        input: userInput,
        candidate_context: JSON.stringify(role_data)  // Add this line
      });
