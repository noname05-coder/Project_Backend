  ...
  try {
    const response = await axios.post(
      ...
      
      {
        ...
        headers: {
          authorization:"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI4YWY3MWExMS1kNGNiLTQwYzUtYWQ2NC03ZWJhMDczMTNiMTIiLCJpYXQiOjE3NDc2NzgzMjEsImV4cCI6MTc0Nzc2NDcyMX0.gz0VuCTz3qCIBd3ed6le4rLGd1U9Tn_Sw4lTnjjcn_k"
        }
      });
...
async function interview() {
  ...
  const displayChatHistory = async() => {
    ...
    try {
      const data = await generate_summery(chatHistory, projectData);
  ...
  try {
    ...
    chatHistory.push({
      ...
      candidate: "",  // This will be filled in on the next iteration
    });
  } catch (error) {
